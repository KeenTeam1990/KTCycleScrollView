...
private let KTPageControlMargin: CGFloat = 15
private let KTPageControlPointWidth: CGFloat = 2
...
extension PageControlAlimentProtocol where Self : UIView
{
    func relayoutPageControl(pageControl: UIPageControl)
    {
        if pageControl.isHidden == false
        {
            ...
            let pageW = CGFloat(pageControl.numberOfPages - 1) * (self.pageControlPointSpace + KTPageControlPointWidth) + KTPageControlPointWidth
            var pageX:CGFloat = 0
            ...
            case .RightBottom:
                pageX = bounds.width - pageW - KTPageControlMargin
            case .LeftBottom:
                pageX = bounds.origin.x + KTPageControlMargin
            }
