//
//  KTNavigationBar.swift
//  KTNavigationBar_swift
//
//  Created by wangrui on 2017/4/19.
//  Copyright © 2017年 wangrui. All rights reserved.

//  超级好用！！！
//  Github地址：https://github.com/wangrui460/KTNavigationBar_swift
...
extension UINavigationBar
{
    ...
    fileprivate func KT_setBackgroundColor(color:UIColor)
    {
    ...
    fileprivate func KT_setBackgroundAlpha(alpha:CGFloat)
    {
    ...
    func KT_setBarButtonItemsAlpha(alpha:CGFloat, hasSystemBackIndicator:Bool)
    {
    ...
    /// 设置导航栏在垂直方向上平移多少距离
    func KT_setTranslationY(translationY:CGFloat)
    {
        transform = CGAffineTransform.init(translationX: 0, y: translationY)
    }
    
    func KT_getTranslationY() -> CGFloat
    {
        return transform.ty
    }
    ...
    open override class func initialize()
    {
        ...
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                let str = ("KT_" + selector.description)
    ...
    // MARK: swizzling pop
    //==========================================================================
    func KT_setTitleTextAttributes(_ newTitleTextAttributes:[String : Any]?)
    {
        ...
        guard let originTitleTextAttributes = titleTextAttributes else {
            KT_setTitleTextAttributes(attributes)
            return
        }
...
extension UINavigationController
{
    ...
    {
        navigationBar.KT_setBackgroundColor(color: barTintColor)
    }

    fileprivate func setNeedsNavigationBarUpdate(barBackgroundAlpha: CGFloat)
    {
        navigationBar.KT_setBackgroundAlpha(alpha: barBackgroundAlpha)
    }
    ...
    open override class func initialize()
    {
        ...
        DispatchQueue.once(token: onceToken)
        {
            ...
            for selector in needSwizzleSelectorArr {
                // _updateInteractiveTransition:  =>  KT_updateInteractiveTransition:
                let str = ("KT_" + selector.description).replacingOccurrences(of: "__", with: "_")
                let originalMethod = class_getInstanceMethod(self, selector)
                let swizzledMethod = class_getInstanceMethod(self, Selector(str))
                method_exchangeImplementations(originalMethod, swizzledMethod)
            }
    ...
    func KT_popToViewController(_ viewController: UIViewController, animated: Bool) -> [UIViewController]?
    {
        ...
        CATransaction.begin()
        let vcs = KT_popToViewController(viewController, animated: animated)
        CATransaction.commit()
        return vcs
    }
    
    // swizzling system method: popToRootViewControllerAnimated
    func KT_popToRootViewControllerAnimated(_ animated: Bool) -> [UIViewController]?
    {
        var displayLink:CADisplayLink? = CADisplayLink(target: self, selector: #selector(popNeedDisplay))
        displayLink?.add(to: RunLoop.main, forMode: .commonModes)
        CATransaction.setCompletionBlock {
        ...
        CATransaction.setAnimationDuration(popProperties.popDuration)
        CATransaction.begin()
        let vcs = KT_popToRootViewControllerAnimated(animated)
        CATransaction.commit()
        return vcs;
    }
    ...
    func KT_pushViewController(_ viewController: UIViewController, animated: Bool)
    {
        ...
        CATransaction.begin()
        KT_pushViewController(viewController, animated: animated)
        CATransaction.commit()
    }
...
extension UINavigationController: UINavigationBarDelegate
{
    ...
    func KT_updateInteractiveTransition(_ percentComplete: CGFloat)
    {
        guard let topViewController = topViewController,
            let coordinator       = topViewController.transitionCoordinator else {
                KT_updateInteractiveTransition(percentComplete)
                return
        }
...
extension UIViewController
{
    ...
    var navBarBarTintColor: UIColor {
        ...
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                ...
                navBar.KT_setBackgroundColor(color: newValue)
            }
            else
            {
    ...
    var navBarBackgroundAlpha:CGFloat {
        ...
        set {
            ...
            if customNavBar.isKind(of: UINavigationBar.self) {
                let navBar = customNavBar as! UINavigationBar
                navBar.KT_setBackgroundAlpha(alpha: newValue)
            }
            else
            {
                if pushToCurrentVCFinished == true && pushToNextVCFinished == false {
    ...
    open override class func initialize()
    {
        ...
        DispatchQueue.once(token: onceToken)
        {
            ...
            {
                let newSelectorStr = "KT_" + selector.description
                let originalMethod = class_getInstanceMethod(self, selector)
    ...
    func KT_viewWillAppear(_ animated: Bool)
    {
        ...
        KT_viewWillAppear(animated)
    }
    
    func KT_viewWillDisappear(_ animated: Bool)
    {
        pushToNextVCFinished = true
        KT_viewWillDisappear(animated)
    }
    
    func KT_viewDidAppear(_ animated: Bool)
    {
...
extension UIColor
{
    ...
    class fileprivate func middleColor(fromColor: UIColor, toColor: UIColor, percent: CGFloat) -> UIColor
    {
        ...
        let neKTed = fromRed + (toRed - fromRed) * percent
        ...
        return UIColor(red: neKTed, green: newGreen, blue: newBlue, alpha: newAlpha)
    }
